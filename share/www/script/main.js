...
;(function(CryptoJS, nanoajax){
  ...
  var get_challenge = function (username) {
  ...
  hash_myhashes = function (myHashes) {
     var serialized = "/index.cgi:"+myHashes["/index.cgi"]+',';
     var cryptoJsDir = "/ext/crypto/";
     var flist = ["core.js", "enc-base64.js", "md5.js", "evpkdf.js", 
                  "cipher-core.js", "aes.js", "sha256.js"];
     for (f in flist) {
         serialized += cryptoJsDir+flist[f] + ':' 
            + myHashes[cryptoJsDir+flist[f]]+ ',';
     }
     serialized += "/script/main.js:" + myHashes["/script/main.js"];
     console.log("serialized hashes ["+ serialized +"]");
     var hash_of_hashes = CryptoJS.SHA256(serialized).toString();
     console.log("hash of hashes ["+hash_of_hashes+"]");
     return hash_of_hashes; 
  }, 
  
  decrypt_form = function (form) {
  ...
  // at this point we have no idea, whether our OTP is correct or not
     ...
     var hash_of_hashes = hash_myhashes(OTPencryptedAjax.myHashes);
    ...
    usernameInput.addEventListener('blur', function(event) { 
      ...
      if ( username.length > 0 ) {
        ...
        console.log("myHashes: "
                      +JSON.stringify(OTPencryptedAjax.myHashes));
      }
    });
